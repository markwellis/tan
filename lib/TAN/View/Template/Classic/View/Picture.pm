...
sub process{
    ...
    $out .= qq\
        ...
        <ul>
            <li>
                Posted by <a href="@{[ $object->user->profile_url ]}" class="TAN-news-user">@{[ $c->view->html($object->user->username) ]}</a>
                @{[
                    ($object->promoted) ? 
    ...
    if ( $c->user_exists && ($c->check_any_user_role(qw/edit_object edit_object_nsfw/) || $c->user->id == $object->user_id) ){
        $out .= qq\
